...
ORDER BY JOB_CODE;
-- SELECT절에 없는 컬럼이라도
-- FROM, JOIN으로 인해 존재하는 컬럼이면 ORDER BY절에 사용 가능
...
SELECT DEPT_TITLE, EMP_NAME, JOB_CODE, J.JOB_CODE , J.JOB_NAME
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- (스칼라 + 상관 쿼리)
SELECT EMP_ID, EMP_NAME, MANAGER_ID, NVL((SELECT EMP_NAME
FROM EMPLOYEE SUB
