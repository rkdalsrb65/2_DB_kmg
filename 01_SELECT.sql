...
-- 주의사항 1) DISTINCT 구문은 SELECT 마다 딱 한번 식만 작성 가능
-- 주의사항 2) DISTINCT 구문은 SELECT 제일 앞에 작성되어야 한다.
-- 주의사항 3) DISTINCT 구문은 SELECT 
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9'; -- 비교 연산 (=) / 대입 연산자 (:=)
...
AND (DEPT_CODE BETWEEN 'D9' OR 'D6')
...
2. 연결연산자 ( || )
3. 비교연산자 ( > < >= <= = != <> )
4. IS NULL / IS NOT NILL, LIE, IN / NOT IN
...
--ORDER BY 3 DESC; (SALARY 가 SELECT 안에 3번째에 있어서 3도 가능)
