...
VIEW 옵션
1. OR REPLACE 옵션 : 생성한 뷰가 존재하면 뷰를 갱신함
2. FORCE/NOFORCE 옵션 : FORCE옵션은 기본 테이블이 존재하지 않더라도 뷰 생성
NOFORCE 옵션이 기본 값으로 지정되어 있음
3. WITH CHECK OPTION 옵션 : 옵션을 설정한 컬럼의 값을 수정 불가능하게 함(삭제는 가능)
4. WITH READ ONLY 옵션 : 뷰에 대해 조회만 가능하고 삽입, 수정, 삭제 등은 불가능하게 함
INLINE VIEW : 일반적으로 FROM절에 사용된 서브쿼리의 결과 화면에 별칭을 붙인 것을
----------------------------------------------------------------
...
INDEX : SQL명령문의 처리 속도를 향상시키기 위해서 컬럼에 대해 생성하는 오라클 객체로
장점 : 검색 속도가 빨라지고 시스템에 걸리는 부하를 줄여 시스템 전체 성능 향상
단점 : 인덱스를 위한 추가 저장 공간이 필요하고 인덱스를 생성하는데 시간이 걸림

INDEX 종류
1. 고유 인덱스(UNIQUE INDEX) : 중복 값이 포함될 수 없음
PRIMARY KEY 제약조건을 생성하면 자동으로 생성됨
2. 비고유 인덱스(NONUNIQUE INDEX) : 빈번하게 사용되는 일반 컬럼을 대상으로 생성
주로 성능 향상을 위한 목적으로 생성
3. 단일 인덱스(SINGLE INDEX) : 한 개의 컬럼으로 구성한 인덱스
4. 결합 인덱스(COMPOSITE INDEX) : 두 개 이상의 컬럼으로 구성한 인덱스
5. 함수 기반 인덱스(FUNCTION-BASED INDEX) : SELECT절이나 WHERE절에 
산술 계산식이나 함수식이 사용된 경우 계산식은 인덱스의 적용을 받지 않음
