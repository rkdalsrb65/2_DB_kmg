...
SELECT A.STUDENT_NO 학번, A.STUDENT_NAME "학생 이름", ROUND(AVG(POINT), 1) "전체 평점"
FROM TB_STUDENT A
JOIN TB_DEPARTMENT B ON (A.DEPARTMENT_NO = B.DEPARTMENT_NO)
JOIN TB_GRADE C ON (A.STUDENT_NO = C.STUDENT_NO)
WHERE DEPARTMENT_NAME = '음악학과'
GROUP BY A.STUDENT_NAME, A.STUDENT_NO
ORDER BY 학번;
...
SELECT B.DEPARTMENT_NAME 학과이름, A.STUDENT_NAME 학생이름, C.PROFESSOR_NAME 지도교수이름
FROM TB_STUDENT A
JOIN TB_DEPARTMENT B ON (B.DEPARTMENT_NO = A.DEPARTMENT_NO)
JOIN TB_PROFESSOR C ON (C.PROFESSOR_NO = A.COACH_PROFESSOR_NO)
...
SELECT A.STUDENT_NO 학번, A.STUDENT_NAME 이름, B.DEPARTMENT_NAME "학과 이름", AVG(C.POINT) 평점
FROM TB_STUDENT A
JOIN TB_DEPARTMENT B ON (B.DEPARTMENT_NO = A.DEPARTMENT_NO)
JOIN TB_GRADE C ON (C.STUDENT_NO = A.STUDENT_NO)
WHERE ABSENCE_YN = 'N'
GROUP BY A.STUDENT_NO, A.STUDENT_NAME, B.DEPARTMENT_NAME
HAVING AVG(C.POINT) >= 4
ORDER BY 학번;

--16. 환경조경학과 전공과목들의 과목 별 평점을 파악할 수 있는 SQL 문을 작성하시오.

SELECT A.CLASS_NO, A.CLASS_NAME, TRUNC(AVG(B.POINT),8)
FROM TB_CLASS A
JOIN TB_GRADE B ON (B.CLASS_NO = A.CLASS_NO)
JOIN TB_DEPARTMENT C ON (C.DEPARTMENT_NO = A.DEPARTMENT_NO)
WHERE C.DEPARTMENT_NAME = '환경조경학과' AND CLASS_TYPE LIKE '전공%'
GROUP BY A.CLASS_NAME, A.CLASS_NO
ORDER BY CLASS_NO;
