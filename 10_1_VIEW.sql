...
--	3) WITH CHECK OPTION 옵션 : 옵션을 설정한 컬럼의 값을 수정 불가능하게 함.(삭제는 가능)
...
-- ORA-01031: 권한이 불충분합니다
	...
	--> 3) GRANT CREATE VIEW TO kh_kmg;

SELECT * FROM V_EMP;
---------------------------------
-- OR REPLACE 확인 + 별칭 --
CREATE OR REPLACE VIEW V_EMP /*(사번, 이름, 부서명, 직급명)*/
AS SELECT EMP_ID, EMP_NAME 이름, DEPT_TITLE 부서명, JOB_NAME 직급명
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
JOIN JOB USING(JOB_CODE);
...
--> ORA-01400: NULL을 ("kh_kmg"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다.
