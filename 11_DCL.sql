...
* 사용자 계정 : 데이터베이스에 대하여 질의, 갱신, 보고서 작성 등의
               ...
               ex) kh_kmg계정(각자 이니셜 계정), workbook 등
*/
...
CREATE USER kmg_sample IDENTIFIED BY sample1234;
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한, .... TO 사용자명;
GRANT CREATE SESSION TO kmg_sample;
...
GRANT CREATE TABLE TO kmg_sample;

ALTER USER kmg_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO kmg_sample;
...
-- kh_kmg  /  kmg_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (sample) kh_kmg 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh_kmg.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 조회 불가

-- 2. (kh) kmg_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO kmg_sample;

-- 3. (sample) 다시 kh_kmg.EMPLOYEE 조회
SELECT * FROM kh_kmg.EMPLOYEE;
...
-- 4. (sample) kh_kmg.EMPLOYEE 테이블을 복사한 테이블 생성
CREATE TABLE EMP_SAMPLE
AS SELECT * FROM kh_kmg.EMPLOYEE;
...
REVOKE SELECT ON EMPLOYEE FROM kmg_sample;
