...
--  조인되는 테이블의 각 행들이 모두 매칭된 데이터가 검색되는 방법(곱집합)
...
AND SUBSTR(EMP_NO, 8, 1) = '2' -- 성별:여자
AND EMP_NAME LIKE '전%'; -- '전'씨 성을 가진 직원  
      
-- 2-1. 이름에 '형'자가 들어가는 직원들의 사번, 사원명, 부서명을 조회하시오.
SELECT EMP_ID, EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE EMP_NAME LIKE '%형%' AND DEPT_CODE = DEPT_ID; -- 부서명

-- 2-2. 이름에 '형'자가 들어가는 직원들의 사번, 사원명, 직급명을 조회하시오.
SELECT EMP_ID, EMP_NAME, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE EMP_NAME LIKE '%형%';  -- 직급명

-- 2-3. 이름에 '형'자가 들어가는 직원들의 사번, 사원명, 직급명, 부서명을 조회하시오.
SELECT EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE
FROM EMPLOYEE
NATURAL JOIN JOB JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
